...
name: frontend-setup
description: |
  Bootstrap Codex for React/Next.js repos: project context, conventions, and
  guardrails so it stops guessing and builds correctly from the first prompt. Use when
  setting up Codex in a frontend codebase.
---

# Frontend Setup — Codex × React/Next.js

You just installed Codex. Your frontend repo is React or Next.js. This skill gives Codex the context it needs to stop guessing and start building correctly from the first prompt.

## Why

Codex writes decent React out of the box. But "decent" means: wrong import paths, outdated patterns, components that don't match your structure, styles that fight your setup. You spend the first hour correcting things that should've been right from the start.

This skill front-loads the project context so Codex knows your stack before you ask it to build anything.

## When to use

Run once when setting up Codex on a frontend repo. Re-run when you change major dependencies (migration to App Router, new styling system, state management swap).
...
Before writing any AGENTS.md rules, tell Codex to scan your project:
...
## Step 2 — Generate your AGENTS.md rules

Based on the scan, add these sections to your project's AGENTS.md. Only include sections relevant to your stack.
...
These rules prevent Codex from doing things that break your project. Add to AGENTS.md:
...
After setting up AGENTS.md, test with a small task:
...
If it got any of these wrong, your AGENTS.md is missing that rule. Add it.

## The principle

Codex with no context writes generic React. Codex with your AGENTS.md writes **your** React. The gap between "works" and "fits" is 15 minutes of setup.
...
2. Generate AGENTS.md sections relevant to the detected stack
3. Include the guardrails section
4. Nothing else — no explanations of what React is, no best practices lectures

The output should be copy-pasteable into AGENTS.md directly.
