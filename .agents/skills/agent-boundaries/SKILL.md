...
Each agent (terminal / repo) gets a `AGENTS.md` that states its boundary. This is the team API — the agent reads it, so you don't have to police it every turn.

### Drop-in AGENTS.md block (per agent):
...
2. **The rules live in the file, not your head.** AGENTS.md is the contract. The agent reads its own boundary — you stop being the reminder.
...
## The concepts (drop these into your own AGENTS.md)

- **bounded context** (DDD) — 1 agent owns 1 domain, no crossing
- **ubiquitous language** (DDD) — same name for the same thing everywhere; keep a glossary in AGENTS.md so agents don't drift
...
- **platform vs stream-aligned** (Team Topologies) — agents do the feature work (stream); you + AGENTS.md are the platform that lets them flow without asking you every step
...
- **Tool-agnostic.** Works with Codex, Cursor, Aider, Copilot — anything that reads a per-project instruction file.
