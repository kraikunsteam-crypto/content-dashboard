...
name: Codex-md-setup
description: |
  Set up a AGENTS.md that stops AI guessing, claiming done without verifying, expanding
  scope past the ask, and forgetting between sessions. Use when configuring Codex
  rules for a repo or workspace.
---

# AGENTS.md Setup — the rules that stop AI guessing, lying, overreaching, and forgetting

AI doesn't misbehave because the model is bad. It guesses when it doesn't know, says "done" before checking, expands scope past what you asked, and forgets everything the next session — because nobody told it not to. The fix isn't a smarter model. It's a AGENTS.md that sets the rules once and loads them every session.

This skill gives you a working AGENTS.md in four layers: **guardrails** (rules that stop the four misbehaviors), **memory** (so it stops repeating mistakes), **spec-driven** (so it never loses project state across /clear or /compact), and **your brain** (so it works the way you do, not the way a generic assistant does).
...
Say: "setup Codex md", "configure my AI rules", "stop AI guessing", "AI keeps forgetting", "Codex md template".

## How AGENTS.md works (30 seconds)

Codex reads `AGENTS.md` from your project root automatically at the start of every session. Whatever's in it becomes standing instruction — no need to repeat yourself. Other agents (Cursor, etc.) have their own equivalent file; the rules below are portable, the filename isn't.
...
These four rules stop the four ways AI misbehaves: **เดา / โกหก / ทำเกิน / ลืม** (guess / lie / overreach / forget — the last one is Layer 2). Drop this block straight into AGENTS.md.
...
**Step 2 —** Add this line near the top of AGENTS.md so the memory loads every session:
...
Open a chat you've used a lot (Codex, ChatGPT, whatever you talk to often) and run these three prompts. Paste the output into AGENTS.md.
...
1. Create `AGENTS.md`, `MEMORY.md`, and `spec.md` in your project root.
2. Paste the **Layer 1 guardrails** block into AGENTS.md.
...
5. Run the **Layer 4 extraction prompts**, paste the output (work patterns / voice / skill triggers) into AGENTS.md.
6. Next session, Codex loads all of it automatically — start with "read spec.md" and it continues where you stopped. When it slips, tell it to log to MEMORY.md.
